...
/// Downloader options
typedef NS_OPTIONS(NSUInteger, RBWebImageDownloaderOptions) {
    /**
     * Put the download in the low queue priority and task priority.
     */
    RBWebImageDownloaderLowPriority = 1 << 0,
    
    /**
     * This flag enables progressive download, the image is displayed progressively during download as a browser would do.
     */
    RBWebImageDownloaderProgressiveLoad = 1 << 1,

    /**
     * By default, request prevent the use of NSURLCache. With this flag, NSURLCache
     * is used with default policies.
     */
    RBWebImageDownloaderUseNSURLCache = 1 << 2,
     ...
     */
    RBWebImageDownloaderIgnoreCachedResponse = 1 << 3,
    
    /**
     * In iOS 4+, continue the download of the image if the app goes to background. This is achieved by asking the system for
     * extra time in background to let the request finish. If the background task expires the operation will be cancelled.
     */
    RBWebImageDownloaderContinueInBackground = 1 << 4,

    /**
     * Handles cookies stored in NSHTTPCookieStore by setting 
     * NSMutableURLRequest.HTTPShouldHandleCookies = YES;
     */
    RBWebImageDownloaderHandleCookies = 1 << 5,

    /**
     * Enable to allow untrusted SSL certificates.
     * Useful for testing purposes. Use with caution in production.
     */
    RBWebImageDownloaderAllowInvalidSSLCertificates = 1 << 6,

    /**
     * Put the download in the high queue priority and task priority.
     */
    RBWebImageDownloaderHighPriority = 1 << 7,
     ...
     */
    RBWebImageDownloaderPreloadAllFrames = 1 << 11,
     ...
     */
    RBWebImageDownloaderMatchAnimatedImageClass = 1 << 12,
};

FOUNDATION_EXPORT NSNotificationName _Nonnull const RBWebImageDownloadStartNotification;
FOUNDATION_EXPORT NSNotificationName _Nonnull const RBWebImageDownloadReceiveResponseNotification;
FOUNDATION_EXPORT NSNotificationName _Nonnull const RBWebImageDownloadStopNotification;
FOUNDATION_EXPORT NSNotificationName _Nonnull const RBWebImageDownloadFinishNotification;

typedef RBImageLoaderProgressBlock RBWebImageDownloaderProgressBlock;
typedef RBImageLoaderCompletedBlock RBWebImageDownloaderCompletedBlock;

/**
 *  A token associated with each download. Can be used to cancel a download
 */
@interface RBWebImageDownloadToken : NSObject <RBWebImageOperation>
 ...
 */
@interface RBWebImageDownloader : NSObject
 ...
 */
@property (nonatomic, copy, readonly, nonnull) RBWebImageDownloaderConfig *config;
 ...
 * @note If you want to modify single request, consider using `RBWebImageContextDownloadRequestModifier` context option.
 */
@property (nonatomic, strong, nullable) id<RBWebImageDownloaderRequestModifier> requestModifier;
 ...
 * @note If you want to modify single response, consider using `RBWebImageContextDownloadResponseModifier` context option.
 */
@property (nonatomic, strong, nullable) id<RBWebImageDownloaderResponseModifier> responseModifier;
 ...
 * @note If you want to decrypt single download data, consider using `RBWebImageContextDownloadDecryptor` context option.
 */
@property (nonatomic, strong, nullable) id<RBWebImageDownloaderDecryptor> decryptor;
 ...
 */
@property (nonatomic, class, readonly, nonnull) RBWebImageDownloader *sharedDownloader;
 ...
 */
- (nonnull instancetype)initWithConfig:(nullable RBWebImageDownloaderConfig *)config NS_DESIGNATED_INITIALIZER;
 ...
 */
- (nullable RBWebImageDownloadToken *)downloadImageWithURL:(nullable NSURL *)url
                                                 completed:(nullable RBWebImageDownloaderCompletedBlock)completedBlock;
 ...
 */
- (nullable RBWebImageDownloadToken *)downloadImageWithURL:(nullable NSURL *)url
                                                   options:(RBWebImageDownloaderOptions)options
                                                  progress:(nullable RBWebImageDownloaderProgressBlock)progressBlock
                                                 completed:(nullable RBWebImageDownloaderCompletedBlock)completedBlock;
 ...
 */
@interface RBWebImageDownloader (SDImageLoader) <RBImageLoader>
