...
typedef void (^RBGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, RBGraphicsImageRendererFormatRange) {
    RBGraphicsImageRendererFormatRangeUnspecified = -1,
    RBGraphicsImageRendererFormatRangeAutomatic = 0,
    RBGraphicsImageRendererFormatRangeExtended,
    RBGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface RBGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) RBGraphicsImageRendererFormatRange preferredRange;
...
@interface RBGraphicsImageRenderer : NSObject
...
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull RBGraphicsImageRendererFormat *)format;
...
- (nonnull UIImage *)imageWithActions:(nonnull NS_NOESCAPE RBGraphicsImageDrawingActions)actions;
