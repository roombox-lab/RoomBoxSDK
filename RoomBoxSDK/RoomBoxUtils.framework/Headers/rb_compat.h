/* rb_compat.h -- Backwards compatible interface for older versions
...
#define RB_COMPAT_VERSION 120

#if RB_COMPAT_VERSION <= 110
#define rb_dos_date dosDate
#else
#define rb_dos_date dos_date
#endif

typedef struct
{
    uint32_t    rb_dos_date;
    struct tm   trb_date;
...
ZEXPORT zipFile zipOpen2_64(const void *path, int append, const char **globalcomment,
    zlib_filefunc64_def *pzlib_filefunc_def);
        zipFile zipOpen_RB(void *stream, int append, const char **globalcomment);
...
ZEXPORT int     zipClose2_64(zipFile file, const char *global_comment, uint16_t version_madeby);
        int     zipClose_RB(zipFile file, const char *global_comment);
        int     zipClose2_RB(zipFile file, const char *global_comment, uint16_t version_madeby);
...
ZEXPORT unzFile unzOpen2_64(const void *path, zlib_filefunc64_def *pzlib_filefunc_def);
        unzFile unzOpen_RB(void *stream);

ZEXPORT int     unzClose(unzFile file);
        int     unzClose_RB(unzFile file);
