...
typedef void(^RBExternalCompletionBlock)(UIImage * _Nullable image, NSError * _Nullable error, RBImageCacheType cacheType, NSURL * _Nullable imageURL);

typedef void(^RBInternalCompletionBlock)(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, RBImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL);

/**
 A combined operation representing the cache and loader operation. You can use it to cancel the load process.
 */
@interface RBWebImageCombinedOperation : NSObject <RBWebImageOperation>
 ...
 */
@property (strong, nonatomic, nullable, readonly) id<RBWebImageOperation> cacheOperation;

/**
 The loader operation from the image loader (such as download operation)
 */
@property (strong, nonatomic, nullable, readonly) id<RBWebImageOperation> loaderOperation;

@end


@class RBWebImageManager;

/**
 The manager delegate protocol.
 */
@protocol RBWebImageManagerDelegate <NSObject>
 ...
 */
- (BOOL)imageManager:(nonnull RBWebImageManager *)imageManager shouldDownloadImageForURL:(nonnull NSURL *)imageURL;
 ...
 */
- (BOOL)imageManager:(nonnull RBWebImageManager *)imageManager shouldBlockFailedURL:(nonnull NSURL *)imageURL withError:(nonnull NSError *)error;
...
[manager loadImageWithURL:imageURL
                  options:0
                 progress:nil
                completed:^(UIImage *image, NSData *data, NSError *error, RBImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                    if (image) {
 ...
 * @endcode
 */
@interface RBWebImageManager : NSObject

/**
 * The delegate for manager. Defaults to nil.
 */
@property (weak, nonatomic, nullable) id <RBWebImageManagerDelegate> delegate;

/**
 * The image cache used by manager to query image cache.
 */
@property (strong, nonatomic, readonly, nonnull) id<RBImageCache> imageCache;

/**
 * The image loader used by manager to load image.
 */
@property (strong, nonatomic, readonly, nonnull) id<RBImageLoader> imageLoader;
...
/**
 ...
 @note This will affect all the load requests for this manager if you provide. However, you can pass `RBWebImageContextImageTransformer` in context arg to explicitly use that transformer instead.
 */
@property (strong, nonatomic, nullable) id<RBImageTransformer> transformer;
 ...
 * @endcode
 */
@property (nonatomic, strong, nullable) id<RBWebImageCacheKeyFilter> cacheKeyFilter;
 ...
 */
@property (nonatomic, strong, nullable) id<RBWebImageCacheSerializer> cacheSerializer;
...
/**
 ...
 @code
 SDWebImageManager.sharedManager.optionsProcessor = [RBWebImageOptionsProcessor optionsProcessorWithBlock:^RBWebImageOptionsResult * _Nullable(NSURL * _Nullable url, RBWebImageOptions options, RBWebImageContext * _Nullable context) {
     // Only do animation on `SDAnimatedImageView`
     if (!context[RBWebImageContextAnimatedImageClass]) {
     ...
     RBWebImageMutableContext *mutableContext = [NSDictionary dictionaryWithDictionary:context];
     mutableContext[RBWebImageContextImageScaleFactor] = @(UIScreen.mainScreen.scale);
     context = [mutableContext copy];
 
     return [[RBWebImageOptionsResult alloc] initWithOptions:options context:context];
 }];
 @endcode
 */
@property (nonatomic, strong, nullable) id<RBWebImageOptionsProcessor> optionsProcessor;
 ...
 */
@property (nonatomic, class, nullable) id<RBImageCache> defaultImageCache;
 ...
 */
@property (nonatomic, class, nullable) id<RBImageLoader> defaultImageLoader;

/**
 * Returns global shared manager instance.
 */
@property (nonatomic, class, readonly, nonnull) RBWebImageManager *sharedManager;
 ...
 */
- (nonnull instancetype)initWithCache:(nonnull id<RBImageCache>)cache loader:(nonnull id<RBImageLoader>)loader NS_DESIGNATED_INITIALIZER;
 ...
 *
 *   The forth parameter is an `RBImageCacheType` enum indicating if the image was retrieved from the local cache
 ...
 */
- (nullable RBWebImageCombinedOperation *)loadImageWithURL:(nullable NSURL *)url
                                                   options:(RBWebImageOptions)options
                                                  progress:(nullable RBImageLoaderProgressBlock)progressBlock
                                                 completed:(nonnull RBInternalCompletionBlock)completedBlock;
...
*/
- (nullable NSString *)cacheKeyForURL:(nullable NSURL *)url context:(nullable RBWebImageContext *)context;
