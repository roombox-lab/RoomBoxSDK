...
#ifndef _RBZIPARCHIVE_H
#define _RBZIPARCHIVE_H
...
extern NSString *const RBZipArchiveErrorDomain;
typedef NS_ENUM(NSInteger, RBZipArchiveErrorCode) {
    RBZipArchiveErrorCodeFailedOpenZipFile      = -1,
    RBZipArchiveErrorCodeFailedOpenFileInZip    = -2,
    RBZipArchiveErrorCodeFileInfoNotLoadable    = -3,
    RBZipArchiveErrorCodeFileContentNotReadable = -4,
    RBZipArchiveErrorCodeFailedToWriteFile      = -5,
    RBZipArchiveErrorCodeInvalidArguments       = -6,
};

@protocol RBZipArchiveDelegate;

@interface RBZipArchive : NSObject
...
+ (BOOL)unzipFileAtPath:(NSString *)path toDestination:(NSString *)destination delegate:(nullable id<RBZipArchiveDelegate>)delegate;
...
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError * *)error
               delegate:(nullable id<RBZipArchiveDelegate>)delegate NS_REFINED_FOR_SWIFT;
...
+ (BOOL)unzipFileAtPath:(NSString *)path
     ...
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
               password:(nullable NSString *)password
                  error:(NSError * *)error
               delegate:(nullable id<RBZipArchiveDelegate>)delegate;
...
@protocol RBZipArchiveDelegate <NSObject>
