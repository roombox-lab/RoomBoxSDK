...
@protocol XDFSimplePingDelegate;

/*! Controls the IP address version used by XDFSimplePing instances.
 */

typedef NS_ENUM(NSInteger, XDFSimplePingAddressStyle) {
    XDFSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    XDFSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    XDFSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface XDFSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<XDFSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) XDFSimplePingAddressStyle addressStyle;
...
/*! A delegate protocol for the XDFSimplePing class.
 */

@protocol XDFSimplePingDelegate <NSObject>

@optional

/*! A XDFSimplePing delegate callback, called once the object has started up.
...
- (void)simplePing:(XDFSimplePing *)pinger didStartWithAddress:(NSData *)address;

/*! A XDFSimplePing delegate callback, called if the object fails to start up.
 *  \details This is called shortly after you start the object to tell you that the 
...
- (void)simplePing:(XDFSimplePing *)pinger didFailWithError:(NSError *)error;

/*! A XDFSimplePing delegate callback, called when the object has successfully sent a ping packet.
 ...
 *      considered API.
 *  \param pinger The object issuing the callback.
 *  \param packet The packet that was sent; this includes the ICMP header (`ICMPXDFHeader`) and the
...
- (void)simplePing:(XDFSimplePing *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;

/*! A XDFSimplePing delegate callback, called when the object fails to send a ping packet.
 *  \details Each call to `-sendPingWithData:` will result in either a 
 *      `-simplePing:didSendPacket:sequenceNumber:` delegate callback or a 
 *      `-simplePing:didFailToSendPacket:sequenceNumber:error:` delegate callback (unless you 
 *      stop the object before you get the callback).  These callbacks are currently delivered 
...
- (void)simplePing:(XDFSimplePing *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;

/*! A XDFSimplePing delegate callback, called when the object receives a ping response.
 ...
 *  \param pinger The object issuing the callback.
 *  \param packet The packet received; this includes the ICMP header (`ICMPXDFHeader`) and any data that
 *      follows that in the ICMP message but does not include any IP-level headers.
 *  \param sequenceNumber The ICMP sequence number of that packet.
 */

- (void)simplePing:(XDFSimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet timeToLive:(NSInteger)timeToLive sequenceNumber:(uint16_t)sequenceNumber timeElapsed:(NSTimeInterval)timeElapsed;

/*! A XDFSimplePing delegate callback, called when the object receives an unmatched ICMP message.
...
- (void)simplePing:(XDFSimplePing *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
...
typedef struct ICMPXDFHeader ICMPXDFHeader;

__Check_Compile_Time(sizeof(ICMPXDFHeader) == 8);
__Check_Compile_Time(offsetof(ICMPXDFHeader, type) == 0);
__Check_Compile_Time(offsetof(ICMPXDFHeader, code) == 1);
__Check_Compile_Time(offsetof(ICMPXDFHeader, checksum) == 2);
__Check_Compile_Time(offsetof(ICMPXDFHeader, identifier) == 4);
__Check_Compile_Time(offsetof(ICMPXDFHeader, sequenceNumber) == 6);

enum {
    XDFICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    XDFICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    XDFICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    XDFICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
